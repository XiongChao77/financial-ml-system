...
class LiveBot:
    def __init__(self):
        self.logger, log_path = common.setup_session_logger(sub_folder=f"market_ml", symbol=LiveConfig.SYMBOL_FTMO)

        self.logger.info("Initializing Live Bot...")

        self._log_startup_info(log_path)
        self.venue = mt5_venue.MT5Venue(
            LiveConfig.mt5_path,
            LiveConfig.SYMBOL_FTMO,
            LiveConfig.MAGIC_NUMBER,
            logger=self.logger,
            firm=Firm.FTMO,
        )
        ...
        self.data_feed = BinanceDataFeed(LiveConfig.SYMBOL_BINANCE, LiveConfig.TIMEFRAME, max_len=self.min_bars_needed + 500)
        # strategy
        self.strategy = MlSignalStrategy(
            ...
            data_interval_ms=self.interval_ms,
        )
    ...
    def run_step(self):
        ...
        position = self.venue.get_current_state()
        self.logger.info(f"MT5 State: Dir={position.dir}, Size={position.size}, " f"Price={position.price}")
        ...
        state = Observation(
            ...
            daily_reset_date=self.venue.get_daily_reset_date(
