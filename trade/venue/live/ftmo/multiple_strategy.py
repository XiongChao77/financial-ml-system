...
def get_base_interval_seconds(intervals: list[int]) -> int:
    ...
    # Greatest common divisor of every interval in seconds
...
class StrategyHolder:
    def __init__(self,strategy_hash,strategy_type:str,path:str,tarin_out_path:str,pre_para:common.BaseDefine, train_para:TrainConfig, st_para:MlStrategyConfig):
        ...
        self.strategy:MlSignalStrategy = None
...
class WindowConfig:
    ...
    def max_window(self) -> int:
        """Largest window size for the current interval"""
...
# ============================================================
def get_bars_since_open(venue, pre_para:common.BaseDefine):
...
def strategy_worker(strategy_hash, strategy_type, path, pre_para:common.BaseDefine, st_para, q):
    ...
    if strategy_type == StrategyType.MT5:
        venue = mt5_venue.MT5Venue(
            path, pre_para.symbol, int(strategy_hash, 16), logger=logger
        )
    else:
        venue = BybitVenue(path, pre_para.symbol)
    ...
    while True:
        ...
        try:
            ...
            logger.info(
                ...
                f"Signal={msg['signal']} Price={msg['price']} bars_held={strategy.bars_held} action={action.action}"
            )
...
class MasterController:
    def __init__(self, strategy_path,debug = True):
        ...
        self.label_col = None
        self.queues = []        # one Queue per subprocess
    ...
    def sleep_until_next_tick(self,base_seconds: int):
        """
        Sleep precisely until the next multiple of base_seconds
        """
        now = time.time()
        # Seconds left until the next slot
        wait_time = base_seconds - (now % base_seconds)
        
        # Add a 0.5s buffer so the exchange data is already updated
    ...
    def run_forever(self):
        ...
        # 1. Compute the GCD of every interval on start-up
        all_intervals = set()
        for trading_type, symbols in self.strategy_input.trading_type.items():
            for symbol, interval_items in symbols.symbols.items():
        ...
        while True:
            ...
            # 3. After waking up, check which intervals are due
            now_ts = int(time.time())
            
            # 1. Snapshot the strategies to run this round
