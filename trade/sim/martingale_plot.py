...
# ============================================================
# Market analysis: real stop-loss counts against a random baseline
# ============================================================
# Direction is the only categorical split here (blue vs orange, an all-pairs
# validated pair); real versus random is carried by line style and a band, so
# no third and fourth hue is needed.
MARKET_SIDES = {
    "long": {"color": "#2a78d6", "label": "Long"},
    "short": {"color": "#eb6834", "label": "Short"},
}
...
def plot_market_analysis(
    ...
    *,
    ...
    """One panel per layer count, real curves against the IID random-walk null."""
    panels = analysis.get("panels", [])
    if not panels:
        logger.warning(f"No market analysis panels to plot, skipping {path}")
        return None

    params = analysis.get("params")
    baseline_model = getattr(params, "baseline_model", "gaussian")
    baseline_label = "Gaussian" if baseline_model == "gaussian" else "Bootstrap"
    deviations = np.asarray(analysis["deviations"], dtype=float) * 100.0
    columns = max(1, min(columns, len(panels)))
    rows = int(np.ceil(len(panels) / columns))

    figure, axes = plt.subplots(
        rows, columns,
        figsize=(7.2 * columns, 3.1 * rows),
    ...
    for axis, panel in zip(flat, panels):
        _style_axis(axis, "Stop losses")
        for side, style in MARKET_SIDES.items():
            key = f"{side}_stops"
            reference = panel["baseline"].get(key)
            if reference is not None:
                # The band is the repeated random-walk spread around the null mean.
                axis.fill_between(
                    deviations, reference["p25"], reference["p75"],
                    color=style["color"], alpha=0.16, linewidth=0,
                )
                axis.plot(
                    deviations, reference["mean"], color=style["color"],
                    linewidth=1.4, linestyle="--",
                )
            axis.plot(
                deviations, panel["real"][key], color=style["color"], linewidth=2.0,
            )
        axis.set_xscale("log")
        axis.set_ylim(bottom=0)
        axis.set_title(
            f"max_layers = {panel['layers']}",
            color=INK, fontsize=10, loc="left", pad=4,
        )

    for axis in flat[len(panels):]:
        axis.set_visible(False)
    for axis in flat[max(0, len(panels) - columns):len(panels)]:
        axis.set_xlabel("price_deviation_pct (%, log)", color=INK_MUTED, fontsize=9)

    handles = [
        Line2D([], [], color=style["color"], linewidth=2.0, label=f"{style['label']} — real")
        for style in MARKET_SIDES.values()
    ] + [
        Line2D([], [], color=style["color"], linewidth=1.4, linestyle="--",
               label=f"{style['label']} — {baseline_label} random walk mean (IQR shaded)")
        for style in MARKET_SIDES.values()
    ]
    figure.legend(
        handles=handles, loc="outside upper right", frameon=False,
        fontsize=9, ncol=2, labelcolor=INK_MUTED,
    )
    figure.suptitle(
        title or f"Stop losses vs price deviation (TP/adverse = layer step): real series against {baseline_label} IID random walks",
    ...
    figure.savefig(path, dpi=140, facecolor=SURFACE)
    plt.close(figure)
    logger.info(f"Market analysis chart saved to {path} ({len(panels)} panels)")
    return path
