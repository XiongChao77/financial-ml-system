...
class ConvLSTM1D_V2(BaseTimeSeriesModel):
    """
    Conv + LSTM hybrid for time-series classification (V2: Multi-task version)
    Added: FeatureSelector support from V1.
    """
    ...
    def __init__(
        ...
        use_feature_selector: bool = False, # [NEW]
    ...
    def forward(self, x: torch.Tensor, lengths: torch.Tensor | None = None, return_fused=False, return_weights=False) -> torch.Tensor:
        ...
        if self.use_feature_selector:
            ...
            x = self.feature_selector(x) # Identity
        ...
        # Task Projections
        feat_trig = self.proj_trigger(feat)
        feat_dir  = self.proj_direction(feat)

        # Heads
        logits_trig = self.head_trigger(feat_trig)
        logits_dir = self.head_direction(feat_dir)
        ...
        # Output logic
        res_logits = (logits_trig, logits_dir)
        ...
        if return_fused:
            ...
            res_logits = (fused_preds, fused_probs)

        if return_weights:
            return res_logits, feature_weights
        return res_logits
    ...
    def export_meta(self, **extra) -> dict:
        return {
            ...
            "use_feature_selector": self.use_feature_selector, # [NEW]
    ...
    def build_from_meta(cls, meta: dict, state: dict, device):
        ...
        model = cls(
            ...
            use_feature_selector=meta.get("use_feature_selector", False), # [NEW]
            in_locked_p=0.0,
            out_locked_p=0.0,
