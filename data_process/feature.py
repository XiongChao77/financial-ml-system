...
class FeatureMAStructure(FeatureBase):
    ...
    def generate(self, df: pd.DataFrame, kline_interval_ms: int):
        ...
        df["MA_BAR_S_L"] = np.log((ma_bs) / (ma_bl))
        df["MA_BAR_M_L"] = np.log((ma_bm) / (ma_bl))
        ...
        df["MA_DAY_S_L"] = np.log((ma_ds) / (ma_dl))

        # WEEK
        w_m, w_l = self.week_windows
        ma_wm = self._ma(close, w_m * kpw)
        ma_wl = self._ma(close, w_l * kpw)
        df["MA_WEEK_M_L"] = np.log((ma_wm) / (ma_wl))

        # === Δ（结构变化率）===
        if self.add_delta:
            df["D_MA_BAR_S_L"] = df["MA_BAR_S_L"].diff().fillna(0.0)
            df["D_MA_DAY_S_L"] = df["MA_DAY_S_L"].diff().fillna(0.0)
    ...
    def _min_history_request(self, kline_interval_ms: int) -> int:
        ...
        base = max(max_bar, max_day, max_week)
...
class FeatureAdvancedVol(FeatureBase):
    ...
    def generate(self, df: pd.DataFrame, kline_interval_ms: int):
        ...
        for w in self.windows:
            # 1. Parkinson: 效率是 ATR 的 5 倍左右
            # 预设常数 1 / (4 * ln2)
            const_p = 1.0 / (4.0 * np.log(2.0))
            df[f'vol_parkinson_{w}'] = np.sqrt(const_p * log_hl.rolling(w).mean())

            # 2. Garman-Klass: 考虑了收盘价位置，效率更高
            const_gk = 2.0 * np.log(2.0) - 1.0
            gk_term = 0.5 * log_hl - const_gk * log_co
            df[f'vol_gk_{w}'] = np.sqrt(gk_term.rolling(w).mean())
...
class FeatureFractalPersistence(FeatureBase):
    ...
    def generate(self, df: pd.DataFrame, kline_interval_ms: int):
        ...
        for w in self.windows:
            # 1. 效率比率 (Efficiency Ratio / Kaufman ER)
            # 净位移 / 路径总长，范围 [0, 1]
            net_change = (close - close.shift(w)).abs()
            total_path = diff.rolling(w).sum()
            df[f'er_{w}'] = np.where(total_path > EPS, net_change / total_path, 0.0)

            # 2. Hurst 指数 (方差尺度法 rolling 实现)
            # Var(k-period return) ∝ k^{2H} => std(k) ∝ k^H => H = log(std_w/std_1) / log(w)
            log_close = np.log(close + EPS)
            ret_1 = log_close.diff()  # 1-period log return
            ret_w = log_close.diff(w)  # w-period log return
            std_1 = ret_1.rolling(w, min_periods=max(3, w // 2)).std()
            std_w = ret_w.rolling(w, min_periods=max(3, w // 2)).std()
            ratio = np.where(std_1 > EPS, std_w / (std_1 + EPS), 1.0)
            raw_h = np.log(np.maximum(ratio, EPS)) / np.log(max(w, 2))
            h_clipped = np.clip(raw_h, 0.0, 1.0)
            df[f'hurst_{w}'] = np.where(np.isfinite(h_clipped), h_clipped, 0.5)
...
class FeatureOrderFlow(FeatureBase):
    """
    成交密度与订单流简化：捕捉买卖失衡与微观结构
    包含：Buy-Sell Imbalance, VPIN Index, Trade Density, POC Bias
    """
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # 建议窗口：14 (日内情绪), 49 (波段重心)
        self.windows = kwargs.get('windows', [14, 49])
        self.features = []
        for w in self.windows:
            self.features.append(f'imbalance_{w}')
            self.features.append(f'vpin_{w}')
            self.features.append(f'trade_density_{w}')
    ...
    def generate(self, df: pd.DataFrame, kline_interval_ms: int):
        ...
        # 1. 瞬时买卖比例 (0.5 为平衡)
        raw_imbalance = np.divide(taker_vol, vol, out=np.full_like(vol, 0.5), where=vol > EPS)

        for w in self.windows:
            # 2. 累积买卖失衡 (平滑处理)
            df[f'imbalance_{w}'] = raw_imbalance.rolling(w).mean() - 0.5

            # 3. 简化 VPIN (指令毒性/流向一致性)
            df[f'vpin_{w}'] = raw_imbalance.rolling(w).std()

            # 4. 成交密度 (当前成交笔数相对于均值的放大倍数)
            trades_ma = trades.rolling(w).mean()
            df[f'trade_density_{w}'] = np.divide(trades, trades_ma, out=np.full_like(trades, 1.0), where=trades_ma > EPS)

            # 5. POC Bias (成交密集区偏离度)
            # 找到窗口内 Volume 最大的索引，并取其 Close 价格
            def get_poc_bias(v_slice, p_slice):
                poc_idx = np.argmax(v_slice)
                poc_price = p_slice[poc_idx]
                return np.log(p_slice[-1] / (poc_price + EPS))

            # 使用 rolling.apply 虽稍慢，但逻辑最稳健
            df[f'poc_bias_{w}'] = vol.rolling(w).apply(
                lambda x: get_poc_bias(x.values, close.iloc[int(x.index[0]):int(x.index[-1])+1].values),
                raw=False
            )
...
class FeatureMomentum(FeatureBase):
    ...
    def generate(self, df: pd.DataFrame, kline_interval_ms: int):
        ...
        # log price for stable log-returns / log-momentum
        log_close = np.log(close + EPS)

        # 1) plain log-momentum
        for h in self.horizons:
            df[f"MOM_{h}"] = (log_close - log_close.shift(h))

        # 2) skip-momentum: skip last k bars (classic anti micro-reversal trick)
        if self.include_skip:
            k = int(self.skip_k)
            horizons_for_skip = self.horizons if (self.skip_horizon is None) else [int(self.skip_horizon)]
            for h in horizons_for_skip:
                # log(close_{t-k} / close_{t-h-k})
                df[f"MOM_{h}_SKIP{k}"] = (log_close.shift(k) - log_close.shift(h + k))

        # 3) vol-adjusted momentum: MOM / realized vol
        if self.include_vol_adj:
            # realized vol of log returns
            log_ret = log_close.diff()
            rv = log_ret.rolling(window=self.vol_window, min_periods=self.vol_window).std()

            horizons_for_va = self.horizons if (self.vol_adj_horizon is None) else [int(self.vol_adj_horizon)]
            for h in horizons_for_va:
                mom = df[f"MOM_{h}"]
                df[f"MOM_{h}_RV{self.vol_window}"] = np.where(
                    rv > EPS,
                    mom / (rv + EPS),
                    np.nan
                )
...
FCOrderFlow = FeatureContainer(FeatureOrderFlow, windows = [14, 49])
