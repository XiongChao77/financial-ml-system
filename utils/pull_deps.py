...
# Try to read the default path configuration
try:
...
class ImportUsageVisitor(ast.NodeVisitor):
    """
    AST visitor:
    1. Records every import statement and the module it maps to.
    2. Records every name that is "loaded" (used) in the code.
    """
    def __init__(self):
        # Import mapping: { variable name: module path }
        # e.g. import utils -> { 'utils': 'utils' }
        # e.g. import utils as u -> { 'u': 'utils' }
        self.imports_map = {} 
        
        # Every name that actually appears in the code
        self.used_names = set()
        
        # Special flag: is there a `from xxx import *`
        self.star_imports = set()

    def visit_Import(self, node):
        for alias in node.names:
            # Record variable name -> module name
    ...
    def visit_ImportFrom(self, node):
        ...
        # Handle a normal from import
        module_name = node.module if node.module else ""
        # Handle the relative import prefix (from .utils import x)
        if node.level > 0:
            module_name = "." * node.level + module_name

        for alias in node.names:
            var_name = alias.asname if alias.asname else alias.name
            # We record the module source here.
            # Note: `from module import func` depends on module
            self.imports_map[var_name] = module_name

    def visit_Name(self, node):
        # Record every name used in a Load context
        if isinstance(node.ctx, ast.Load):
            self.used_names.add(node.id)
    
    def visit_Attribute(self, node):
        # For obj.prop we need obj to be recorded as used
        self.generic_visit(node)

def analyze_file_imports(file_path):
    """
    Parse a file and return the set of { 'module name' }.
    Only the modules that are actually used in the code are returned.
    """
    try:
    ...
    # 1. Always include star-imported modules (static analysis cannot tell)
    active_modules.update(visitor.star_imports)

    # 2. Special case for __init__.py
    # Imports in __init__.py usually exist to re-export, so keep them even when unused internally
    is_init = os.path.basename(file_path) == '__init__.py'

    # 3. Compare the import table with the usage table
    for var_name, module_name in visitor.imports_map.items():
        # Either an __init__.py, or the name is used in the code
        if is_init or var_name in visitor.used_names:
            active_modules.add(module_name)
        # else:
        #     print(f"  [unused] ignored in {os.path.basename(file_path)}: {module_name} (alias: {var_name})")

    return active_modules

def index_project_files(root_dir):
    """Build the file index: module.name -> file_path"""
    file_map = {}
    ...
    for dirpath, _, filenames in os.walk(abs_root):
        for fname in filenames:
            ...
            # Convert into a module name
            base_name = os.path.splitext(rel_path)[0]
            mod_name = base_name.replace(os.sep, '.')
            
            file_map[mod_name] = full_path
            
            # Handle packages (folder/__init__.py -> folder)
...
def find_project_dependencies(script_path, project_root):
    ...
    while queue:
        ...
        # === core change: use the smart analysis function ===
        imported_modules = analyze_file_imports(current_file)
        
        # Package path needed to resolve the relative imports
        # Package path of the current file relative to root
        ...
        for mod in imported_modules:
            # Handle the relative imports (e.g. from . import x)
            if mod.startswith('.'):
                if current_pkg:
                    # Simple relative path resolution
                    # Note: a rigorous relative import resolution is complex, this is simplified
                    # .utils -> pkg.utils, ..utils -> parent.utils
                    dots = len(mod) - len(mod.lstrip('.'))
                    base_mod = mod.lstrip('.')
                    # This resolution may be imperfect but works for most flat layouts
            ...
            # 2. Try the parent (from a.b import c -> may be a/b.py)
            if not target_file:
...
def copy_files(file_list, target_dir):
    ...
    for src in file_list:
        ...
        if os.path.exists(dst) and not os.path.samefile(src, dst):
            ...
            # print(f"name clash: {filename} -> {new_name}")
...
def main():
    ...
    for f in files:
        # Print the relative path, easier to read
